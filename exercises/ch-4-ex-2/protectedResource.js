...
app.get('/words', getAccessToken, requireAccessToken, function(req, res) {
	...
	if(__.contains(req.access_token.scope, 'read')) {
		...
		res.set('WWW-Authnticate', 'Bearer realm=localhost:9002, error="insufficent_scope", scope="read"');
		res.status(403).end();
	}
});
...
app.post('/words', getAccessToken, requireAccessToken, function(req, res) {
	if(__.contains(req.access_token.scope, 'write')) {
		...
		res.set('WWW-Authnticate', 'Bearer realm=localhost:9002, error="insufficent_scope", scope="write"');
		res.status(403).end();
	}
});
...
app.delete('/words', getAccessToken, requireAccessToken, function(req, res) {
	...
	if(__.contains(req.access_token.scope, 'write')) {
		...
		res.set('WWW-Authnticate', 'Bearer realm=localhost:9002, error="insufficent_scope", scope="delete"');
		res.status(403).end();
	}
});
